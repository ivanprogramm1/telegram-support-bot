...
async def admin_commands(
    ...
    await message.answer(
        ...
        "/allow ID баланс — добавить старого реферала и его баланс вручную\n"
        "/broadcast текст — отправить текстовую рассылку всем пользователям\n"
        "/check_inactive — проверить и удалить неактивных из каналов\n\n"
        "Кнопки в панели:\n"
        "📊 Статистика — показать количество UID и пользователей\n"
        "📣 Рассылка — отправить любое сообщение: текст, голосовое, видео, фото\n"
...
async def allow_referral(
    ...
    uid = parts[1].strip() if len(parts) >= 2 else ""
    balance_text = parts[2].strip() if len(parts) >= 3 else "1"
    balance = parse_money(balance_text)


    if not uid or not uid.isdigit() or balance <= 0:

        await message.answer(
            "Использование:\n"
            "/allow ID баланс\n\n"
            "Например:\n"
            "/allow 127671970 1411.54\n\n"
            "Если баланс не знаешь, можно так:\n"
            "/allow 127671970"
        )
    ...
    await message.answer(
        ...
        f"Баланс в таблице: {balance_amount}\n"
        "Теперь он сможет пройти сценарий и проверку депозита."
    )
...
async def admin_broadcast_start(
    ...
    await message.answer(
        ...
        "Можно отправить текст, голосовое, видео, фото, кружок или документ."
    )
...
async def admin_broadcast_send(
    ...
    sent = await send_message_broadcast(message)
...
async def send_message_broadcast(message: Message):
